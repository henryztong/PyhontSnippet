...
将数据库查询结果以一定的格式输出
输出结果：
Credit表字段：apply_no:tt20190717181650
Credit表字段：created:1563358619
Credit表字段：user_id:230661
...
from object_model.sp_grant_credits import Credit
from object_model.sp_grant_user_contacts import UserContact
from object_model.sp_grant_credit_opr import CreditOpr
from object_model.sp_grant_credit_user import CreditUser
from object_model.sp_grant_credit_user_company import CreditUserCompany
from object_model.sp_grant_credit_user_relation import CreditUserRelation
from object_model.sp_grant_store import Store
from object_model.sp_grant_user_photos import UserPhoto
import mysql.connector


class func_DataSQL(object):
	"""和包数据库操作 """
	# 连接数据库并实例化一个对象,格式：'数据库类型+数据库驱动名称://用户名:口令@机器地址:端口号/数据库名'
...
if __name__ == '__main__':
	...
	# table2_name = UserContact
	# table2_column1 = UserContact.credit_id

	table2_name = CreditOpr
	table2_column1 = CreditOpr.id
	table2_column2 = CreditOpr.opr_id


	table3_name = Store
	table3_column = Store.opr_id
